...
class RF_model(random_forest):
    ...
    def load_data(self,opt_model_type):
        ...
        for i in range(self.available_instances):
            with open(os.path.join(self.instance_dir,f'instance {i}.txt')) as f:
                for line in f:
                    ...
                    param = line[0]
                    if param in X_dict:
                        X_dict[param][i,:] = np.asarray([int(v) for v in line[1:]])
                    if param in self.fixed_params:
                        self.fixed_params[param][0,:] = np.asarray([int(v) for v in line[1:]])

                    # if line[0] == 'n':
                    #     continue
                    # elif line[0] == 'W':
                    #     if 'W' in X_dict:
                    #         X_dict['W'][i,:] = int(line[1])
                    #     elif 'W' in Fixed_dict:
                    #         Fixed_dict['W']
                    #     # X[i,-1] = int(line[1])
                    #     # Fixed[i, -1] = int(line[1])
                    # elif line[0] == 'v':
                    #     # X[i,0:50] = np.asarray([int(v) for v in line[1:]])
                    #     # Fixed[i, 0:50] = np.asarray([int(v) for v in line[1:]])
                    # elif line[0] == 'w':
                    #     # X[i,50:100] = np.asarray([int(v) for v in line[1:]])
                    #     # X[i, 0:50] = np.asarray([int(v) for v in line[1:]])

        Y = np.zeros((self.available_instances,50))
        for i in range(self.available_instances):
        ...
        # TODO: Might need to come back and fix this later on
        X = np.concatenate(tuple(X_dict.values()))
        # Fixed = np.concatenate(tuple(Fixed_dict.values()),dim=1)

        self.n_out = 50
        self.n_features = X.shape[1]

        split_index1 = int(self.available_instances*0.8)
        split_index2 = int(self.available_instances*0.9)

        X_train = X[:split_index1,:]
        X_test = X[split_index1:split_index2,:]
        X_val = X[split_index2:,:]

        Y_train = Y[:split_index1, :]
        Y_test = Y[split_index1:split_index2, :]
        Y_val = Y[split_index2:, :]

        self.input_dict = {'TRAIN': X_train,
                           'TEST': X_test,
                           'VAL': X_val,
                           'PARAM IDX': {'w': list(range(50))}}

        self.output_dict = {'TRAIN': Y_train,
                           'TEST': Y_test,
                           'VAL': Y_val}

        self.norm_groups = {'w': (X.max(),X.min())}

        # Define the normalisation groups for the input data and the factors for normalisation
        # self.norm_groups = {(0,50): (X[:,0:50].max(),X[:,0:50].min()),
        #                     (50,101): (X[:,50:].max(),X[:,50:].min())}
    ...
    def print_problem_metrics(self, X, Y, coeff):
        ...
        shared = np.sum(Y * self.pred, axis=1)
        shared_percent = 100 * shared/np.sum(Y,axis=1)
    ...
    # def load_data(self,opt_model_type):
    #
    #     X = np.zeros((self.available_instances,101))
    #
    #     for i in range(self.available_instances):
    #         with open(os.path.join(self.instance_dir,f'instance {i}.txt')) as f:
    #             for line in f:
    ...
    #                 Y[i,w] = 1
    #
    #     split_index = int(self.available_instances * 0.8)
    #
...
class NN_model(neural_network):
    ...
    def load_data(self,opt_model_type,rng_seed=8983):
        ...
        # Only important if there is some ordering in the way the data was generated
        rng = np.random.default_rng(seed=rng_seed)
        p = rng.permutation(self.available_instances)
        # p = np.random.permutation(self.available_instances)
...
class ANN(nn.Module):
    def __init__(self,n_features,n_out,model_params):
        ...
        if n_features <= 600:
            ...
            self.model_details['Layers'] = f'{n_features} 512 256 {n_out}'
